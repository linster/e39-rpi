package ca.stefanm.ibus.car.data
...
open class IBusMessage(
    ...
    fun toWireBytes() : ByteArray {
        return with(Buffer()) {
            ...
            val length = 1 /* Dest is 1 byte */ + data.toByteArray().size + 1 /* Checksum */
            writeByte(length)
            ...
            var checksum = 0x00
            this.copy().readByteArray().forEach { byte -> checksum = checksum xor byte.toInt() }
            writeByte(checksum)
        }.readByteArray()
    }
